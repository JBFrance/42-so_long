...
/*   Updated: 2023/04/10 16:58:40 by jfrances         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	change_rem(t_parse *pr)
{
	...
	if (!pr->spr.rem)
		pr->spr.rem = mlx_xpm_file_to_image(pr->mlx, "sprites_xpm/mine.xpm", \
		&pr->spr.img_width, &pr->spr.img_height);
	if (i % 2 == 0)
	{
	...
	{
		mlx_destroy_image(pr->mlx, pr->spr.rem);
		pr->spr.rem = mlx_xpm_file_to_image(pr->mlx, "sprites_xpm/mine1.xpm", \
		&pr->spr.img_width, &pr->spr.img_height);
	}
	++i;
}
...
void	sprite_pos_check(t_parse *pr, int j, int i)
{
	...
	else if (pr->map[i][j] == 'B')
		mlx_put_image_to_window(pr->mlx, pr->win, \
			pr->spr.rem, j * 64, i * 64 + 35);
}

void	update_frame(t_player *pl, t_parse *pr, int i, int j)
{
	mlx_clear_window(pr->mlx, pr->win);
	i = -1;
	while (pr->map[++i])
	{
		j = -1;
...
void	check_pos(t_player *pl, t_parse *pr)
{
	...
	if (pr->map[pl->x][pl->y] == 'B')
	{
		free_matrix(pr->map, 0, "");
		game_exit("Warning: Mines Explode");
	}
